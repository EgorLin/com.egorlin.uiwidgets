...
namespace EgorLin.UIWidgets.Essentials.Tutorial.Runtime.Types.Actions {
    ...
    public struct WaitListFill : IAction {
        ...
        public ActionResult Execute(in Context context) {
            ...
            var list = context.window as ListComponent;
            if (this.tag.isList == true) {
                ...
                if (this.tag.ignoreSearch == true) {
                        ...
                        foreach (var moduleBase in list.componentModules.modules) {
                            if (moduleBase is TutorialListComponentModule module) {
                                if (module.uiTag == this.tag.uiTag) {
                                    list = module.listComponent as ListComponent;
                                }
                            }
                        }
                    }
                    if (list != null) {
                        var tagModule = list.GetModule<TutorialWindowComponentTagComponentModule>();
                        if (tagModule != null) {
                    ...
                    list = context.window.FindComponent<ListComponent, Closure>(state, static (state, x) => {
