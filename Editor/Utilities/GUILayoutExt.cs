...
namespace EgorLin.UIWidgets.Editor.Utilities {
    ...
    public static class GUILayoutExt {
						    ...
						    if (GUILayout.Button("Generate", GUILayout.Width(80f), GUILayout.Height(30f)) == true) {
							    ...
							    if (PrefabUtility.IsPartOfAnyPrefab(obj) == true) {
								    ...
								    using (var edit = new EditPrefabAssetScope(path)) {
									    ...
									    edit.prefabRoot.transform.GetComponent<WindowLayout>().safeZone = EditorHelpers.AddSafeZone(edit.prefabRoot.transform);
								    ...
								    root.transform.GetComponent<WindowLayout>().safeZone = EditorHelpers.AddSafeZone((Transform)safeZoneRectTransform.objectReferenceValue ?? root.transform);
        ...
        public static void PropertyField(SerializedProperty property, System.Func<EditorParametersRegistry, bool> regCheck = null) {

	        var hasAnyReg = false;
	        var description = string.Empty;
	        var holders = new System.Collections.Generic.List<string>();
	        var holdersObjs = new System.Collections.Generic.List<IHolder>();
	        if (regCheck != null) {
		        ...
		        foreach (var target in property.serializedObject.targetObjects) {
			        ...
			        if (target is WindowObject windowObject) {

				        var registry = WindowObjectRegistry.GetRegistry(windowObject);
				        if (registry != null) {
        ...
        public static void Icon(string path, float width = 32f, float height = 32f) {
	        ...
	        icon.normal.background = global::UnityEditor.Experimental.EditorResources.Load<Texture2D>(path);
        ...
        public static void DrawHideBehaviour(SerializedProperty hideBehaviour, SerializedProperty hideBehaviourOneByOneDelay) {
	        var prevFlags = (HideBehaviour)hideBehaviour.enumValueFlag;
	        var flags = (HideBehaviour)EditorGUILayout.EnumFlagsField(hideBehaviour.displayName, prevFlags);
	        if (prevFlags != flags) {
	        ...
	        if (((HideBehaviour)hideBehaviour.enumValueFlag & HideBehaviour.OneByOne) != 0) {
        ...
        public static void DrawShowBehaviour(SerializedProperty showBehaviour, SerializedProperty showBehaviourOneByOneDelay) {
	        ...
	        if ((ShowBehaviour)showBehaviour.enumValueIndex == ShowBehaviour.OneByOne) {
        ...
        public static void GetRandomValue(SerializedProperty randomValueProperty, out float from, out float to) {
	        var rnd = randomValueProperty.FindPropertyRelative(nameof(AnimationParameters.RandomValue.random));
	        var value = randomValueProperty.FindPropertyRelative(nameof(AnimationParameters.RandomValue.randomFromTo));
