...
namespace EgorLin.UIWidgets.Editor.Drawers {

    [CustomPropertyDrawer(typeof(Tweener.EaseFunction))]
    public class EaseAnimationParametersPropertyDrawer : PropertyDrawer {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
            ...
            if (EditorGUI.DropdownButton(position, GUIContent.none, FocusType.Passive, EditorStyles.toolbarDropDown) == true) {
                ...
                PopupWindow.Show(position, new EaseEditorWindow((Tweener.EaseFunction)property.enumValueIndex, durationValue, delayValue) {
                    onSelect = (ease) => {
            ...
            EaseEditorWindow.Item.DrawProgress(new Rect(position.x + 1f, position.y + 1f, position.width - 20f, position.height - 2f), (Tweener.EaseFunction)property.enumValueIndex, -1f, null, null, lbl, false);
    ...
    public class EaseEditorWindow : PopupWindowContent {
        ...
        public System.Action<Tweener.EaseFunction> onSelect;
        ...
        public class Item {
            ...
            private readonly Tweener.EaseFunction type;
            ...
            public Item(Tweener.EaseFunction type, Vector2 duration, Vector2 delay, bool selected) {
                this.type = type;
                ...
                {
                    var attribute = memInfo[0].GetCustomAttribute(typeof(Tweener.GroupAttribute), false);
                    if (attribute is Tweener.GroupAttribute groupAttribute) {
                        this.groupHeader = groupAttribute.caption;
                    }
                    this.isGroup = attribute != null;
                }
                {
                    var attribute = memInfo[0].GetCustomAttribute(typeof(Tweener.InspectorOrderAttribute), false);
                    if (attribute is Tweener.InspectorOrderAttribute orderAttribute) {
            ...
            public static void DrawProgress(Rect rect, Tweener.EaseFunction func, float progress, float? currentDuration, float? currentDelay, string caption, bool selected) {

                const float sizeX = 10f;
                const float sizeY = 10f;
                const int precision = 100;

                var funcEase = Tweener.EaseFunctions.GetEase(func);
            ...
            public Tweener.EaseFunction GetTypeValue() {
                return this.type;
            }
        ...
        public EaseEditorWindow(Tweener.EaseFunction selected, Vector2 duration, Vector2 delay) {
            var values = System.Enum.GetValues(typeof(Tweener.EaseFunction));
            this.items = new System.Collections.Generic.List<Item>();
            foreach (var value in values) {
                var func = (Tweener.EaseFunction)value;
    ...
    [CustomPropertyDrawer(typeof(AnimationParameters.ShowHideParameters))]
    ...
    [CustomPropertyDrawer(typeof(AnimationParameters.RandomValue))]
